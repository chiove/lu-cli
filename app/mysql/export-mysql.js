...
const mysql = `mysqldump -u ${USERNAME} -p`;
...
const main = async () => {
  ...
  if (result.type === 'library') {
    shell.exec(`${mysql} ${DATABASE} > ${DATABASE}.sql`);// 导出库
  } else {
    ...
    shell.exec(`${mysql} ${DATABASE} ${res.name}> ${res.name}.sql`);// 导出单个sql
  }
};
